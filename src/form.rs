...
pub fn init_form(name: &str,history: &JsValue) {
    ...
    let root = document.get_element_by_id("root").expect("page `root` exists");

    root.set_inner_html("");

    let form_node: web_sys:: Element= document.create_element("form").expect("DOM element to have been created");
